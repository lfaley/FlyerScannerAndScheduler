...
4. **`summarize`'s `failureRate`** (`index.html:3402`) counts a fell-back call twice in the denominator — one user-visible operation, two log rows. **SETTLED AND FIXED, v10.6 — it needed no answer from Logan.** Verified by execution: 12 operations (10 clean, 1 failed outright, 1 rescued by the fallback) reported `failureRate` **0.0769** against a true **0.0833** — understated by 7.7%, and the error scales with how often the desktop is asleep, so it is worst exactly when someone is reading this number to find out why. `failed` already counted operations while the denominator counted rows: two units. `summarize` now returns `operations` (rows minus rescues) and divides by that; `calls` still counts rows, because two API calls really were made. **A first attempt at confirming this used an unreachable fixture** — a `fellBackTo` row paired with a FAILED Anthropic row, which showed a 2x distortion. `callAI` only sets `fellBackTo` after the Anthropic call resolves, so that log cannot occur; a fallback that also fails logs two plain failures. The real defect is the smaller one above.
