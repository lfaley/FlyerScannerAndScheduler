...
1. ~~**Document it as a bearer secret**~~ **DONE 23 Aug.** Written into
   **GMAIL-WATCHER-SETUP.md** as two new sections: *"The URL and the token
   together are a password"* (why the token cannot move out of the URL, with the
   JSONP/CORS reason cited to `gmail-watcher.gs:543-547`, and a warning not to
   "fix" it) and *"Changing the token (rotation)"* — a seven-step click path with
   checkpoints. **Correction to this plan's wording: rotation does NOT need a
   redeploy.** The script reads `SECRET` fresh on every call (`:561`), so a
   property change takes effect immediately; only code changes need a new
   deployment version.
